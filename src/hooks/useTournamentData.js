    ...
    const newData = {
      ...
      export const useTournamentData = () => {
        ...
        const addReward = (playerId, reward) => {
          ...
          // Misma lógica que addRouletteReward
          if (reward.includes('Objetos de Tienda') || (reward.includes('Objetos') && reward.includes('Tienda'))) {
            ...
            const existingInsurances = player.rewards?.filter(r => r.text?.includes('🛡️ Seguro de Muerte') || (typeof r === 'string' && r.includes('🛡️ Seguro de Muerte'))) || [];
